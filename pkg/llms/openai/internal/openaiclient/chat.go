...
func parseStreamingChatResponse(ctx context.Context, r *http.Response, payload *ChatRequest) (*ChatCompletionResponse,
	...
	go func() {
		...
		for scanner.Scan() {
			...
			if err != nil {
				streamPayload.Error = errors.Wrap(err, "error decoding streaming response")
		...
		if err := scanner.Err(); err != nil {
			responseChan <- StreamedChatResponsePayload{Error: errors.Wrap(err, "error reading streaming response")}
			return
		}
	}()
	// Combine response
...
func combineStreamingChatResponse(
	...
	for streamResponse := range responseChan {
		...
		if payload.StreamingFunc != nil {
			...
			if err != nil {
				return nil, errors.Wrap(err, "streaming func returned an error")
			}
		}
		if payload.StreamingReasoningFunc != nil {
			err := payload.StreamingReasoningFunc(ctx, reasoningChunk, chunk)
			if err != nil {
				return nil, errors.Wrap(err, "streaming reasoning func returned an error")
			}
		}
	}
	return &response, nil
