...
func (mc *MessageContent) UnmarshalJSON(data []byte) error {
	...
	for _, part := range m.Parts {
		...
		case "binary":
			...
			if err != nil {
				return errors.Wrap(err, "failed to decode binary data")
			}
		...
		default:
			return errors.Newf("unknown content type: '%s'", part.Type)
		}
	}
...
func (tc *TextContent) UnmarshalJSON(data []byte) error {
	...
	if m["type"] != "text" {
		return errors.Newf("invalid type for TextContent: %v", m["type"])
	}
	tc.Text = m["text"]
	return nil
}
...
func (iuc *ImageURLContent) UnmarshalJSON(data []byte) error {
	...
	_, ok := m["type"].(string)
	if !ok {
		return errors.New(`missing "type" field in ImageURLContent`)
	}
	imageURL, ok := m["image_url"].(map[string]any)
	if !ok {
		return errors.New("invalid image_url field in ImageURLContent")
	}
	url, ok := imageURL["url"].(string)
	if !ok {
		return errors.New("invalid url field in ImageURLContent")
	}
...
func (bc *BinaryContent) UnmarshalJSON(data []byte) error {
	...
	if m["type"] != "binary" {
		return errors.Newf("invalid type for BinaryContent: %v", m["type"])
	}
	binary, ok := m["binary"].(map[string]any)
	if !ok {
		return errors.New("invalid binary field in BinaryContent")
	}
	mimeType, ok := binary["mime_type"].(string)
	if !ok {
		return errors.New("invalid mime_type field in BinaryContent")
	}
	encodedData, ok := binary["data"].(string)
	if !ok {
		return errors.New("invalid data field in BinaryContent")
	}
...
func (tc *ToolCall) UnmarshalJSON(data []byte) error {
	...
	if !ok {
		return errors.New("invalid id field in ToolCall")
	}
	typ, ok := toolCall["type"].(string)
	if !ok {
		return errors.New("invalid type field in ToolCall")
	}
	var fc FunctionCall
	fcData, ok := toolCall["function"].(json.RawMessage)
	if ok {
		if err := json.Unmarshal(fcData, &fc); err != nil {
			return errors.Wrap(err, "error unmarshalling function call")
		}
	}
	tc.ID = id
	tc.Type = typ
...
func (tc *ToolCallResponse) UnmarshalJSON(data []byte) error {
	...
	if m["type"] != "tool_response" {
		return errors.Newf("invalid type for ToolCallResponse: %v", m["type"])
	}
	tr, ok := m["tool_response"].(map[string]any)
	if !ok {
		return errors.New("invalid tool_response field in ToolCallResponse")
	}
	toolCallID, ok := tr["tool_call_id"].(string)
	if !ok {
		return errors.New("invalid tool_call_id field in ToolCallResponse")
	}
	name, ok := tr["name"].(string)
	if !ok {
		return errors.New("invalid name field in ToolCallResponse")
	}
	content, ok := tr["content"].(string)
	if !ok {
		return errors.New("invalid content field in ToolCallResponse")
	}
